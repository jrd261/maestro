function [A,dA,F,dF,T,dT] = mnlinsinfit_errorestimate(X,Y,F)
...
Z = Y - sum(AA.*sin(2*pi*FF.*(XX-TT)),2);
[f1,a1] = mdft(X,Y,min(F)-20/max(X),max(F)+20/max(X));
[f,a] = mdft(X,Z,min(F)-20/max(X),max(F)+20/max(X));
newy = midft(f1,a1,X);
R2_Post = sum((Z-newy).^2);           
