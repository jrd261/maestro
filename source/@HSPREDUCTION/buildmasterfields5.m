function buildmasterfields5(HSPReduction)
...
% Some things need to be initialized. The configuration needs to be loaded
% as we will use it quite frequently. An array to hold the master
% field and a structure for storing geometric solutions will also be
% initialized. 
...
% of the objects found on images.
MasterField = [];
...
for iFITSObject = randperm(length(FITSList))
    ...
    CTG = FITSList(iFITSObject).findstarsdev; 
    if isempty(CTG), 
    ...
    if isempty(MasterField) || isempty(MasterField.Geometry)
        ...
        MasterField.Signal = FITSList(iFITSObject).SkyRemovedPrimaryImage;
    ...
    for NPS = min([size(CTG,2),size(CMG,2),MaestroConfiguration.MAX_SOURCES_PER_PERMUTATION]):-1:2
        ...
        for iSolution = 1:min(1,length(goodSolutionIndicies))
            ...
            Z2 = FITSList(iFITSObject).SkyRemovedPrimaryImage*mmRatio;            
