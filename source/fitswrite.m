function fitswrite(data,filename,header)
...
header(strcmp(header(:,1),'SIMPLE'),:) = [];
header(strcmp(header(:,1),'BITPIX'),:) = [];
header(strcmp(header(:,1),'NAXIS'),:) = [];
header(strcmp(header(:,1),'NAXIS1'),:) = [];
header(strcmp(header(:,1),'NAXIS2'),:) = [];
header(strcmp(header(:,1),'BZERO'),:) = [];
header(strcmp(header(:,1),'BSCALE'),:) = [];
header(strcmp(header(:,1),'EXTEND'),:) = [];

[nrow,ncol]=size(data);
def = [make_card('SIMPLE','T');       ...
      make_card('BITPIX',-32);       ...
      make_card('NAXIS',2);          ...
...
header_cards = char(zeros(size(header,1),80));
for i=1:size(header,1)-1
    header_cards(i,:) = make_card(header{i,1},header{i,2});
end

header_cards(i+1,:) = make_card('END');

header_cards = [def;header_cards;];
